...
function readPackages(root, errors) {
  ...
  return names.slice(0, MAX_PACKAGES).map((key) => {
    const spec = typeof declared[key] === 'string' ? declared[key] : null;
    const installed = installedManifest(root, key);
    // Un alias (« compat-react: npm:react@18 ») s'installe sous son nom local mais EST react : c'est ce nom-là que
    // le registre et les avis connaissent (audit du 25/09/2026, n° 10). Le manifeste installé fait foi, sinon le spec.
    const aliased = spec && spec.startsWith('npm:') ? spec.slice(4).replace(/@[^@]*$/, '') : null;
    const real = installed && typeof installed.name === 'string' && PACKAGE_NAME.test(installed.name) ? installed.name : (aliased && PACKAGE_NAME.test(aliased) ? aliased : key);
    const entry = { name: real, version: installed ? installed.version : null, declared: spec };
    if (real !== key) entry.alias = key;
    return entry;
  });
}

/** Le manifeste installé d'un paquet (nom réel, version) : son package.json dans le node_modules le plus proche, en remontant un peu. */
function installedManifest(root, name) {
  ...
  for (let depth = 0; depth <= MAX_DEPTH; depth++) {
    ...
    try {
      ...
      if (manifest && typeof manifest.version === 'string') return { name: manifest.name, version: manifest.version };
    } catch {
      // pas là : on remonte
    }
...
function handle(request) {
  ...
  // Le nonce de la requête revient dans le corps signé : une réponse ne vaut que pour la requête qui l'a demandée (audit n° 6).
  return respond({ probe_version: LANTERN_VERSION, key_id: key.id, collected_at: new Date().toISOString(), nonce: request.header('x-lamparo-nonce'), facts, errors }, key.secret);
}
